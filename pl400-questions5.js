var pl400Questions5 = [
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'The correct sequence for creating a follow-up task in a plug-in is: (1) Get the target account from InputParameters using context.InputParameters["Target"], (2) Create the task entity and set regardingobjectid using account.ToEntityReference() to create a proper entity reference, (3) Use service.Create() to insert the new task record. Using PreEntityImages is incorrect because the account is being created (not updated). Using account directly without ToEntityReference() would fail as regardingobjectid requires an EntityReference type.',
  },
  {
    id: 5039,
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'To package a PCF control for deployment: (1) pac solution init creates the solution project structure with publisher details, (2) pac solution add-reference links the PCF control project to the solution, (3) msbuild /t:build /restore compiles the solution and creates the deployable .zip file. npm commands (build, start) are for local development and testing, not for creating deployment packages. The pac pcf init command is for creating new controls, not packaging existing ones.',
  },
  {
    id: 5100,
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'The correct approach for Azure Functions connecting to Dataverse without passwords is using managed identity or application users. However, the given choices describe creating a Microsoft Entra ID user with Non-interactive access mode. Non-interactive users authenticate via S2S (server-to-server) without passwords. The pac admin assign-user command assigns security roles to the user in Dataverse. Note: Managed identity would be the preferred modern approach but is not among the given options.',
  },
  {
    id: 5173,
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'To remove a component from a managed solution in production: (1) Navigate to Solutions in the development environment, (2) Find and delete the business rule from the solution, (3) Export as managed and import to production - the upgrade will remove the deleted component. Creating a new solution in production does not help. Deactivating the rule does not remove it from the solution. The solution upgrade mechanism handles component deletions when the source solution no longer contains them.',
  },
  {
    id: 5194,
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'Environment variables allow different values per environment without code changes: (1) Create an empty environment variable named Custom Text Placeholder that power users can set, (2) Create a function to retrieve the environment variable value and display it in notifications, (3) Export the solution for distribution. Power users can set the environment variable value in their testing environments. Config pages and JSON configuration add unnecessary complexity. Default values are optional but the empty variable enables customization.',
  },
  {
    id: 5240,
  ...
  },
  {
    ...
    correct: [0, 1, 2],
    explanation: 'To configure a webhook for Azure Functions: (1) Select Register New Web Hook to create a new webhook endpoint, (2) Enter the endpoint URL from the Azure Function (found via Get function URL in Azure portal), (3) Register a step for Create of SalesOrder to trigger the webhook. Service Endpoints are for Service Bus, not HTTP endpoints. HttpHeader authentication passes the webhook key in headers. The step registration binds the webhook to specific Dataverse events.',
  },
  {
    id: 5287,
